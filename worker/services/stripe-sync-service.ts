...
export class StripeSyncService {
  ...
  /**
   * Lean payments-page sync: pulls active subscriptions + recent one-time
   * charges, upserts into the `payments` table. Idempotent.
   */
  async syncPayments(
    projectId: string,
    ...
    const apiKey = await this.stripeService.getDecryptedKey(projectId);
    if (!apiKey) throw new Error("Stripe not connected for this project");
    ...
    // ─── Subscriptions ────────────────────────────────────────────────
    const subs = await this.listAllSubscriptions(apiKey);
    for (const sub of subs) {
      const cust = await this.getCachedCustomer(apiKey, sub.customer, customerCache);
      const item = sub.items?.data?.[0];
      const unit = item?.price?.unit_amount ?? 0;
      const interval = item?.price?.recurring?.interval ?? "month";
      const intervalCount = item?.price?.recurring?.interval_count ?? 1;
      const monthly = normalizeToMonthly(unit, interval, intervalCount);
      const planName = item?.price?.nickname || item?.price?.product || null;
      const result = await paymentService.upsertFromExternal({
        projectId,
        source: "stripe",
        externalPaymentId: sub.id,
        kind: "subscription",
    ...
    // ─── One-time charges (no invoice = not subscription-driven) ──────
    const ninetyDaysAgo = Math.floor(Date.now() / 1000) - 90 * 24 * 60 * 60;
    const charges = await this.listChargesSince(apiKey, ninetyDaysAgo);
    for (const ch of charges) {
      if (ch.invoice) continue; // subscription-driven, skip
      if (ch.status !== "succeeded") continue;
      if (!ch.customer) continue;
      const cust = await this.getCachedCustomer(apiKey, ch.customer, customerCache);
      const result = await paymentService.upsertFromExternal({
        projectId,
        source: "stripe",
        externalPaymentId: ch.id,
        kind: "one_time",
        customerEmail: cust.email,
        customerName: cust.name,
        planName: ch.description ?? null,
        mrr: ch.amount / 100,
        startedAt: new Date(ch.created * 1000),
        status: ch.status,
      });
      if (result.inserted) inserted++;
      else updated++;
  ...
  private async getCachedCustomer(
    apiKey: string,
    customerId: string,
    cache: Map<string, { email: string | null; name: string | null }>,
    ...
    if (!response.ok) {
      ...
      cache.set(customerId, empty);
      return empty;
    }
    const c = (await response.json()) as StripeCustomer;
    const out = { email: c.email ?? null, name: c.name ?? null };
    cache.set(customerId, out);
    return out;
  }

  private async listAllSubscriptions(apiKey: string): Promise<
    Array<
      StripeSubscription & {
        start_date?: number;
        items?: {
    ...
    >
  > {
    const all: Array<StripeSubscription & { start_date?: number }> = [];
    let startingAfter: string | undefined;
    let hasMore = true;
    while (hasMore) {
      const params = new URLSearchParams({
      ...
      const data = (await r.json()) as {
        data: Array<StripeSubscription & { start_date?: number }>;
        has_more: boolean;
      };
      all.push(...data.data);
      hasMore = data.has_more;
  ...
  private async listChargesSince(
    apiKey: string,
    sinceTimestamp: number,
  ): Promise<Array<StripeCharge & { description?: string | null }>> {
    const all: Array<StripeCharge & { description?: string | null }> = [];
    let startingAfter: string | undefined;
    let hasMore = true;
    while (hasMore) {
      const params = new URLSearchParams({
      ...
      if (!r.ok) break;
      const data = (await r.json()) as {
        data: Array<StripeCharge & { description?: string | null }>;
        has_more: boolean;
      };
      all.push(...data.data);
      hasMore = data.has_more;
...
function normalizeToMonthly(
  ...
  const cents = unitAmountCents;
  const perBilling = cents / 100;
  const months =
    interval === "month"
      ? intervalCount
      : interval === "year"
        ...
        : interval === "week"
          ...
          : interval === "day"
            ? intervalCount / 30
            : 1;
  return months > 0 ? perBilling / months : perBilling;
}
